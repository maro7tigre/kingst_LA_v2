...
void init_simulation(py::module_ &m) {
    ...
    py::return_value_policy::reference);
        ...
        if (!remote_frame) {
            for (int byte_idx = 0; byte_idx < dlc; byte_idx++) {
    ...
    //------------------------------------------------------------------
    // Python-specific protocol generators (higher-level helpers)
    //------------------------------------------------------------------
    ...
    sim_group.def("add_uart_simulation", [](SimulationChannelDescriptorGroup &self, 
        ...
        // Add each byte
        for (U8 byte : data) {
            // Using the previously defined add_uart_byte
            if (with_start_bit) {
                // Start bit is always LOW
                uart_tx->Transition(); // Transition to LOW
                uart_tx->Advance(bit_width);
